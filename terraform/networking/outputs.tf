output "vpc_id" {
  description = "VPC ID. Consumed by #2 (RDS), #3 (Airflow on EC2), #4 (cd-api Lambda)."
...
output "rds_security_group_id" {
  description = "Security group to attach to the RDS instance (#2)."
  value       = aws_security_group.rds.id
}

output "airflow_security_group_id" {
  description = "Security group to attach to the Airflow EC2 instance (#3)."
