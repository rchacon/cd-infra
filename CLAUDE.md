...
- **NAT gateway is off by default** (`enable_nat_gateway = false` in `networking/`) -- nothing needs outbound internet from a private subnet until `airflow/`'s EC2 instance or `rds/`'s bootstrap actually requires it. Flip it on in whichever PR first needs it, not preemptively.
