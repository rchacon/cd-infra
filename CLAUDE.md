...
`bootstrap/`'s `observability_readonly_role_arn` output is a separate, Terraform-managed IAM role (read-only CloudTrail/CloudWatch/Cost Explorer) for answering AWS usage/billing questions -- deliberately *not* folded into `cd-terraform`'s own policy above, to keep that one minimal and reactively-built rather than preemptively broadened for unrelated investigation work. `cd-terraform` (or any future identity) needs one narrow `sts:AssumeRole` statement scoped to that ARN added to its own policy before it can actually assume the role -- the same "add exactly what's needed, empirically" discipline, just a one-line addition instead of a dozen new read actions.
